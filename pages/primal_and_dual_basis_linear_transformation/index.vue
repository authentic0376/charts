<template>
  <div class="app-container">
    <h1>2D Basis Transformation (Nuxt + KaTeX)</h1>
    ...
    <ClientOnly>
      <div ref="controlsContainerRef" id="controls-container">
        <span>e₁.x</span><div ref="slider_e1x_container"></div>
        <span>e₁.y</span><div ref="slider_e1y_container"></div>
        <span>e₂.x</span><div ref="slider_e2x_container"></div>
        <span>e₂.y</span><div ref="slider_e2y_container"></div>
        <div ref="resetButtonContainer" style="grid-column: 3 / span 2; justify-self: end;"></div>
      </div>
      <template #fallback>
    ...
    <div id="info-container">
      <div class="info-section"><h3>Primal Basis</h3><span ref="el_primal_e1"></span><span ref="el_primal_e2"></span></div>
      <div class="info-section"><h3>Transformation Matrix</h3><span ref="el_matrix_m"></span><span ref="el_determinant"></span></div>
      <div class="info-section"><h3>Dual Basis</h3><span ref="el_dual_epsilon1"></span><span ref="el_dual_epsilon2"></span></div>
...
onMounted(async () => {
  try {
    ...
    if (canvasContainerRef.value && controlsContainerRef.value) { // controlsContainerRef 확인 추가
      ...
      // 초기 KaTeX 렌더링
      updateInfoDOM(calculated, { el_primal_e1, el_primal_e2, el_matrix_m, el_determinant, el_dual_epsilon1, el_dual_epsilon2, el_matrix_m_inv });
    } else { console.error("DOM containers for p5 not ready after dynamic import and nextTick."); }

  } catch (error) { console.error("Failed to load p5.js or initialize sketch:", error); }
});
